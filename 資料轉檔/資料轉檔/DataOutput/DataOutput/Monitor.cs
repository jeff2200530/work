    ...
    public class Monitor : MainProcessorBase
    {
        ...
        public void FileCreated(object sender, FileSystemEventArgs e)
        {
            ...
            if (!e.FullPath.EndsWith(".txt"))
            {
                ...
                string folderName1 = Path.GetFileNameWithoutExtension(e.FullPath);
                //GetFolder(e.FullPath);
                if (_source.Contains(folderName))
                {
                    ...
                    logger.Write(Level.trace, $"FileCreated開始執行  資料夾：{folderName}，檔案：{fileName}", "", "");
                    #endregion
                    ...
                    #region Log
                    //logger.Trace($"FileCreated結束執行，資料筆數{files.Length}");
                    logger.Write(Level.trace, $"FileCreated結束執行，資料筆數{files.Length}", "", "");
                    //Console.WriteLine($"資料夾：{folderName}，檔案：{fileName} 資料筆數{_process._count} 執行完成!");
                    #endregion
                }
                if (_source.Contains(folderName1))
                {
                    string[] D1 = Directory.GetDirectories(e.FullPath);
                     foreach (var folder1 in D1)
                        if (Path.GetFileNameWithoutExtension(folder1) == "file")
                        {
                            //Console.WriteLine($"取得file{folder1}");
                            string[] D2 = Directory.GetDirectories(folder1);
                            foreach (var folder2 in D2) {

                                DirectoryInfo dirInfo = new DirectoryInfo(folder2);
                                Thread.Sleep(3000);
                                ...
                                if (files.Length > 0)
                                    _process.Execute(new Request() { file = files, table = folderName1 });

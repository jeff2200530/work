...
namespace DataOutput.Insert
{
    public abstract class InsertBase : MainProcessorBase
    {
        ...
        public DataTable TxtToDt(string path)
        {
            ...
            logger.Trace($"開始讀取日期：{folderName}，分公司{filaName}資料");
            try
            {
                ...
                while ((nextLine = sr.ReadLine()) != null)
                {
                    ...
                    catch
                    {
                        ...
                        logger.Error($"日期：{folderName}，分公司{filaName}，第{count + 1}筆欄位不符");
                        continue;
                    }
                }
                ...
                logger.Trace($"日期：{folderName}，分公司{filaName}，轉換成功{count}筆,失敗{fail}筆");
                return dt;
            }
            catch
            {
                ...
                _writer._logQueue.Enqueue(new LogFormat() { content = _logMessage.ToString(), path = "\\log.txt" });
                logger.Error($"日期：{folderName}，分公司{filaName}，讀取失敗");
                return null;
            }
        }
